...
const mockPathExists =
  jest.fn<(path: string, revision: string) => Promise<boolean>>()
  ...
  describe('createElement', () => {
    ...
    describe('Given deep path (pathAfterType.length >= 2)', () => {
      describe('When type directory is in path without intermediate folders', () => {
        ...
        it('Given StaticResource nested file, When creating element, Then should probe and find component root', async () => {
          // Arrange
          const path =
        ...
        it('Given ExperienceBundle nested file, When creating element, Then should probe and find component root', async () => {
          // Arrange
          const path =
      ...
      describe('When intermediate folders exist between type dir and component', () => {
        ...
        it('Given ObjectTranslation with nesting, When creating element, Then should find correct component root', async () => {
          ...
          mockPathExists.mockImplementation(p =>
            Promise.resolve(
              p ===
                'force-app/main/default/objectTranslations/nested/Account-es/Account-es.objectTranslation-meta.xml'
            )
          )
        ...
        it('Given Bot with nesting, When creating element, Then should probe and find component root', async () => {
          // Arrange
          const path =
    ...
    describe('When directoryName is NOT in path (git scan fallback)', () => {
      ...
      it('Given sibling name not in path, When creating element, Then should skip and continue walking', async () => {
        // Arrange
        const path = 'force-app/main/any/path/MyResource/images/logo.png'
        const revision = 'HEAD'
        mockListDirAtRevision.mockResolvedValueOnce(['logo.png'])
        mockListDirAtRevision.mockResolvedValueOnce([
          'images',
        ...
        // Assert - skips 'Other' because not in path, finds 'MyResource'
        expect(element.componentName).toBe('logo')
        expect(mockListDirAtRevision).toHaveBeenCalledTimes(3)
      })
    ...
    describe('When probing finds component root at intermediate depth', () => {
      it('Given nested StaticResource, When probing, Then should find component root by meta file existence', async () => {
        // Arrange
        const path =
        ...
        mockPathExists.mockImplementation(p =>
          Promise.resolve(
            p ===
              'force-app/main/default/staticresources/UpdateStaticResourceFile/UpdateStaticResourceFile.resource-meta.xml'
          )
        )
    ...
    describe('edge cases', () => {
      it('Given no probe matches, When creating element with typeDir in path, Then should fallback to last segment', async () => {
        // Arrange
        const path =
      ...
      it('Given no typeDir in path, When creating element, Then should fallback via walk-up', async () => {
        // Arrange
        ...
        // Assert - falls back to last segment via walk-up
        expect(element.componentName).toBe('file')
        expect(mockListDirAtRevision).toHaveBeenCalled()
      })
