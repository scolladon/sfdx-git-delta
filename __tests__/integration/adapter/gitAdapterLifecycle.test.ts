...
// Every case here spawns the real CLI as a child process. On a cold
// Windows runner that costs seconds per spawn, well past vitest's 5s
// default, so the budget is stated rather than inherited.
...
describe('Given the built sfdx-git-delta CLI', () => {
  describe('When generating a delta manifest end-to-end', () => {
    it(
      'Then the process exits clean with no dangling FileHandle warning',
      async () => {
    ...
    it(
      'Then a --source-dir containing a wildcard is rejected with a non-zero exit',
      async () => {
        // Arrange
        const outputDir = await trackedTempDir('sgd-lifecycle-output-')
    ...
    it(
      'Then the unmatched-scope warning reports the rev the user typed, not its resolved SHA',
      async () => {
