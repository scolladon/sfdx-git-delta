...
class LightningHandler extends StandardHandler {
  ...
  handleAddtion(){
    ...
    this.promises.push(fse.copy(path.join(this.config.repo,this.splittedLine.slice(0,-1).join(path.sep)), path.join(this.config.output,this.splittedLine.slice(0,-1).join(path.sep))));
  }
