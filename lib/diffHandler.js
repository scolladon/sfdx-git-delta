      ...
      child.on('close', code => {
        ...
        .forEach(line => {
          ...
          if(line.startsWith('D') && !line.endsWith(METAFILE_SUFFIX) && !line.endsWith('.xml')) {
            ...
            try {
              fse.copySync(`${this.config.repo}/${line}`, explodedLine.join('/'))
              if(metadata[xmlType].metaFile === true) {
                  fse.copySync(`${this.config.repo}/${line}${METAFILE_SUFFIX}`, `${explodedLine.join('/')}${METAFILE_SUFFIX}`)
              }
              if(~['aura','lwc'].indexOf(xmlType)) {
                console.log(xmlType);
                let a = fse.copySync(`${this.config.repo}/${line.split('/').slice(0,-1).join('/')}`,`${explodedLine.slice(0,-1).join('/')}`);
              } 
            } catch(err) {
              console.log(err);
              // Do Nothing
