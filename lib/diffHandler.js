  ...
  diff() {
    return new Promise((resolve, reject) => {
      ...
      child.on('close', code => {
        ...
        .forEach(line => {
          const xmlType = line.split(path.sep).reduce((p,v)=> metadata.hasOwnProperty(v) ? v : p  ,'')
          const splittedLine = line.split(path.sep)
          ...
          if(line.startsWith('D') && ((metadata[xmlType].metaFile === true && !line.endsWith(METAFILE_SUFFIX)) || metadata[xmlType].metaFile === false)) {
            ...
            const source = path.normalize(~['aura','lwc'].indexOf(xmlType) ? path.join(this.config.repo,splittedLine.slice(0,-1).join(path.sep)) : path.join(this.config.repo,line));
            const target = path.normalize(~['aura','lwc'].indexOf(xmlType) ? path.join(this.config.output,splittedLine.slice(0,-1).join(path.sep)) : path.join(this.config.output,line));
            ...
            if(xmlType === 'staticresources') {
              ...
              .forEach(src => {
                promises.push(fse.copy(path.normalize(path.join(srcPath,src)),path.normalize(path.join(targetPath,src))))
              })
            } else {
              promises.push(fse.copy(source, target));
              if(metadata[xmlType].metaFile === true) {
                promises.push(fse.copy(source+METAFILE_SUFFIX, target+METAFILE_SUFFIX))
              }
            }
          } 
        })
        Promise.all(promises.map(p => p.catch(e => console.warn(err.message))))
        .then(()=>resolve(diffs))
      });
