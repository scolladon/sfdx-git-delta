  ...
  diff() {
    return new Promise((resolve, reject) => {
      ...
      child.on('close', code => {
        ...
        fullResult.join('')
        .split('\n')
        .filter(line=>!line.endsWith(METAFILE_SUFFIX) && line.split('/').some(part=>metadata.hasOwnProperty(part)))
        .reduce((promise,line) => {
          const explodedLine = line.split('/');          
          const xmlType = explodedLine.reduce((p,v)=> metadata.hasOwnProperty(v) ? v : p  ,'')

          if(line.startsWith('D') && !line.endsWith('.xml')) {
            ...
            try {
              const source = path.normalize(~['aura','lwc'].indexOf(xmlType) ? `${this.config.repo}/${line.split('/').slice(0,-1).join('/')}` : xmlType === 'staticresources' ? `${this.config.repo}/${line.match(/.*\/staticresources\/([^\/]*)+/)[0]}` : `${this.config.repo}/${line}`);
              const target = path.normalize(~['aura','lwc'].indexOf(xmlType) ? `./${explodedLine.slice(0,-1).join('/')}` : xmlType === 'staticresources' ? `${explodedLine.join('/').match(/.*\/staticresources\/([^\/]*)+/)[0]}` :  `./${explodedLine.join('/')}`);
