...
export default mergeConfig(
  ...
  defineConfig({
    test: {
      ...
      // The spawned CLI looks the API version up only when none is pinned, and
      // this bucket runs behind an unreachable proxy, so the pin keeps a git
      // assertion from turning into an API-version refusal. NN.0 is mandatory:
      // the orgApiVersion flag rejects a bare integer before the command runs.
