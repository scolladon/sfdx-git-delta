...
- Resolves each of `--from`/`--to` to the commit it peels to (`git rev-parse --verify <ref>^{commit}` semantics, `GitAdapter.resolveCommit`): symbolic refs resolve and annotated tags are followed through their chain to the tagged commit, whose full SHA is written back to `config.from`/`config.to`; a revision that resolves but does not peel to a commit (`HEAD^{tree}`, `HEAD:path`, a bare tree or blob oid, a tag pointing at a tree) is rejected with `error.ParameterIsNotCommit`, naming the flag and the value as typed, while an unresolvable one keeps `error.ParameterIsNotGitSHA` and its fetch-depth hint; both flags are validated identically, before the merge-base lookup, the diff and any tree walk
