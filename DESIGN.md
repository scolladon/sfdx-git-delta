...
**Error mapping**: `mapTsgitError` (`src/adapter/tsgitErrorMap.ts`) normalizes tsgit's exceptions to sgd's expected error shape at each adapter method boundary — `parseRev`, `getFirstCommitRef`, `streamDiffLines` (via its internal `requestDiff`), `getBufferContent`, `getStringContent`, and `getMergeBase`. `grepUnderPaths`, `grepMatchingPathspecs`, and `buildTreeIndex` debug-log and degrade instead of throwing, consistent with the warnings-not-exceptions policy below. The mapper is a pure function of the operation context and the repository path: it checks for object-not-found first, then walks a code-keyed repository-refusal table, then falls back to a generic passthrough, rendering refusal messages from sgd's own repository path (tsgit renders paths basename-only) and returning a typed `RepositoryRefusalError` so `ConfigValidator` can report it directly instead of the vaguer per-ref `ParameterIsNotGitSHA`.

`peelToCommit(oid, label)` — shared by `indexRevision` and `getMergeBase` — follows a resolved oid down through any tag chain to the commit it ultimately points at. `revParse` returns the tag **object** oid for an annotated tag rather than auto-peeling, and tsgit's `mergeBase` primitive does not peel either; without this step, `--from v1.0.0 --merge-base` would compare the tag object against a commit and falsely report no merge base.
