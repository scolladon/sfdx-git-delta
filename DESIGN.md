...
- **IncludeProcessor**: handles `--include` and `--include-destructive` flags. Lists every file under `config.source` at `config.to` through the run's `to` index (built under `config.source` scope whenever an include file is set, in every mode), filters through the include patterns, then processes the matching lines through `DiffLineInterpreter` as synthetic additions/deletions: additions run with `{ from: firstCommit, to: config.to }`, deletions with `{ from: config.to, to: firstCommit }`, where `withRevisions` (`src/types/runContext.ts`) overrides the revisions and carries `trees` through unchanged. The DELETION re-entry relies on the reader's degrade: `firstCommit` is normally unindexed, so a container's liveness check answers `false` and the forced line is classified as a deletion. **Known residual**: when `firstCommit` coincides with `config.from` or `config.to` (a run whose `--from` is the initial commit — README's documented programmatic default), that revision *is* indexed, the liveness check answers `true`, and a forced deletion of a live container lands in `package.xml` instead of `destructiveChanges.xml`. It predates the mode-independent index build and holds equally in both modes; pinned as a live characterisation test in `__tests__/integration/manifestParityAcrossGenerateDelta.test.ts` and tracked as a follow-up.
