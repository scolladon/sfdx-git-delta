...
- Container manifest entry: the container is listed in `package.xml` only when the to-side has a surviving add or modify in any bucket — including registry-excluded children and scalar/unknown tags, neither of which produce their own manifest entry. A delete-only diff lists nothing in `package.xml` and emits no delta file; it produces `destructiveChanges.xml` entries only for children that are not registry-excluded
...
`GitAdapter` (`src/adapter/GitAdapter.ts`) wraps `git` via `child_process.spawn` with a singleton pattern keyed by the composite string `<repo>\0<to>` — so spread copies of the same config (for example `ioExecutor`'s per-revision `{...config, to: rev}`) share one adapter instance and one `git cat-file` subprocess rather than spawning a new one per call. All git invocations go through one of three internal seams: `_spawnLines` for line-streamed output, `_gitBuffered` for one-shot full-stdout calls, and dedicated streaming methods (`streamArchive`, `streamContent`, `GitBatchCatFile`). It minimizes git subprocess spawns via two strategies:
