...
The strict failure contract that `metadataDiff` depends on (txml is otherwise tolerant — `<Root><unclosed>` parses to a partial tree without throwing) is preserved by `xmlBalanceValidator.validateXml`: a one-pass O(N) scanner that walks the prologue, comments, CDATA, processing instructions, and element nesting on a stack, throwing `MalformedXmlError` on tag mismatches, unclosed tags, multiple roots, or unterminated literals. Quote-aware tag-end finder so attribute values containing `>` (legal in XML 1.0) don't fool the boundary scan. `parseToSidePropagating` runs the validator; `parseFromSideSwallowing` skips it because its caller already discards parse errors and falls back to "no prior content", and txml's tolerant partial parse is fine there — saves one O(N) scan per from-side parse.
